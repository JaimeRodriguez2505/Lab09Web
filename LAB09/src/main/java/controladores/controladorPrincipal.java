    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            if (accion.equals("bienvenida")) {
                ...
                if (request.getParameter("boton").compareTo("GRABAR") == 0) {
                    String operacion = request.getParameter("operacion");
                    String strDateIni = request.getParameter("xfechaini");
                    String strDateFin = request.getParameter("xfechafin");
                    ...
                    curso.setSesiones(Integer.parseInt(request.getParameter("xsesiones")));
                    curso.setCapacidad(Integer.parseInt(request.getParameter("xcapacidad")));
                    curso.setInscritos(Integer.parseInt(request.getParameter("xsins")));
                ...
                if (request.getParameter("boton").compareTo("GRABAR") == 0) {
                    ...
                    if (rpta) {
                        out.print("<br><h2>Registro grabado correctamente!</h2>");
                    } else {
                        out.print("<br><h2>Error al grabar matricula !</h2>");
                    }
                }
            } else if (accion.compareTo("matriculaMostrarMatricula") == 0) {
                if (request.getParameter("modo").compareTo("Lista") == 0) {
                    ...
                    request.getRequestDispatcher("/operaciones/matriculaMostrarMatricula.jsp").forward(request, response);

                } else if (request.getParameter("boton").compareTo("Buscar") == 0) {
                    String nroDoc = request.getParameter("xalu");
                    List<Matricula> arrMatriculas = new ArrayList<Matricula>();
                    Matricula matricula = new Matricula();
                    IMatriculaDAO dao = new MatriculaDAOImpl();

                    matricula.setNroDoc(nroDoc);
                    arrMatriculas = dao.buscarMatricula(matricula);
                    request.setAttribute("arrMatriculas", arrMatriculas);
                    request.getRequestDispatcher("/operaciones/matriculaMostrarMatricula.jsp").forward(request, response);
